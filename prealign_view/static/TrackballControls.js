...
THREE.TrackballControls = function ( object, domElement ) {
	...
	var _this = this;
	var STATE = { NONE: -1, ROTATE: 0, ZOOM: 1, PAN_OR_GRAB: 2, TOUCH_ROTATE: 3, TOUCH_ZOOM: 4, TOUCH_PAN: 5 };
	...
	function mousedown( event ) {
		...
		} else if(_state === STATE.PAN_OR_GRAB) {
			if(_this.grab_mode) {
				var grab_continue = false;
				if(_this.on_grab_start) {
					grab_continue = _this.on_grab_start(_this.getMouseNDC(event.clientX, event.clientY));
				}

				if(grab_continue) {
					_grab_prev = _this.getMouseNDC(event.clientX, event.clientY);
				} else {
					_state = STATE.NONE;
				}
			} else {
	...
	function mousemove( event ) {
		...
		if ( _state === STATE.ROTATE && !_this.noRotate ) {
			...
			if(_this.grab_mode) {
				var curr = _this.getMouseNDC(event.clientX, event.clientY);
				var delta = curr.clone().sub(_grab_prev);
				_grab_prev = curr;

				if(_this.on_grab_move) {
					_this.on_grab_move(curr);
				}
			} else {
	...
	function mouseup( event ) {
		...
		if(_state === STATE.PAN_OR_GRAB && _this.grab_mode) {
			if(_this.on_grab_end) {
				_this.on_grab_end();
			}
		}
